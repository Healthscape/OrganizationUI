...
export class UpdateInfoComponent {
    ...
    constructor(private specialtyService: SpecialtyService, _tokenService: TokenService, private router: Router, private userService: UserService, private snackBar: MatSnackBar) {
        ...
        if (!this.me) {
            userService.getDetailedMe().subscribe({
                next: (user) => {
                    this.me = user;
                }
            })
